function [CellsTable,diffTable] = cell_tracking_v1_simple(CellsTable, composite_differences, time_column_name)
  ...
  for timepoint=1:length(composite_differences)
    ...
    previous_timepoint = timepoint+min(CellsTable{:,time_column_name})-1;
    current_timepoint = timepoint+min(CellsTable{:,time_column_name}); 
    ...
    %% RESEGMENTED CELLS 
    if timepoint>1
        lostCellsTable = CellsTable(CellsTable.TraceUsed==0 & CellsTable{:,time_column_name}==previous_timepoint-1,:);
        currentTimepointTable = CellsTable(CellsTable{:,time_column_name}==current_timepoint & strcmp(CellsTable.Trace,'None'),:);
    ...
    % newborns_cells = find(CellsTable.SaddlePoint > 50 & CellsTable{:,time_column_name}==current_timepoint & strcmp(CellsTable.Trace,'None'));
    ...
    % previous_timepoint_cells = CellsTable{:,time_column_name}==previous_timepoint;
    ...
    % Give a trace ID to cells that were not matched
     cells_entering_frame = CellsTable{:,time_column_name}==current_timepoint & strcmp(CellsTable.Trace,'None');
