...
%     end
    ...
    elseif ismember(app.plates(plate_num).metadata.ImageFileFormat, {'XYZCT-Bio-Formats'})
      ...
      pos = numimages_label.Position;
      numimages_label.Position = [pos(1) 276 pos(3) pos(4)]; % Move up because some fields will be missing for Bio-formats
      pos = app.plates(plate_num).NumberOfImagesField.Position;
      app.plates(plate_num).NumberOfImagesField.Position = [pos(1) 249 pos(3) pos(4)]; % Move up because some fields will be missing for Zeiss
    end

    % Plate Map Table
    well_table = uitable(tab,'Data',plate.wells,'Visible', Plate_Map_Table_visibility, 'Position',[163,15,624,254], ...
      'ColumnEditable',true, 'RowName',letters);
    well_label = uilabel(tab, 'Text', 'Plate Map:', 'Visible', Plate_Map_Table_visibility, 'Position', [163,276,73,20], 'FontSize', 14, 'FontName', 'Yu Gothic UI');

    % Filter Data Title
    filter_label = uilabel(tab, 'Text', 'Filter Input:', 'Visible', Filter_Data_visibility, 'Position', [15,276,93,20], 'FontSize', 14, 'FontName', 'Yu Gothic UI');

    % Filter Columns
    rows_label = uilabel(tab, 'Text', 'Rows:', 'Visible', Filter_Row_visibility, 'Position', [48,248,34,20], 'FontSize', 12, 'FontName', 'Yu Gothic UI');
    app.plates(plate_num).filter_rows = uieditfield(tab, 'Visible', Filter_Row_visibility, 'Position', [87,247,56,22], ...
      'UserData', plate_num, ...
      'ValueChangedFcn', createCallbackFcn(app, @changed_FilterInput_, true) ...
    );

    % Filter Columns
    columns_label = uilabel(tab, 'Text', 'Columns:', 'Visible', Filter_Columns_visibility, 'Position', [26,219,56,20], 'FontSize', 12, 'FontName', 'Yu Gothic UI');
    app.plates(plate_num).filter_columns = uieditfield(tab, 'Visible', Filter_Columns_visibility, 'Position', [87,218,56,22], ...
      'UserData', plate_num, ...
      'ValueChangedFcn', createCallbackFcn(app, @changed_FilterInput_, true) ...
    );

    % Filter Fields
    fields_label = uilabel(tab, 'Text', 'Fields:', 'Visible', Filter_Fields_visibility, 'Position', [43,190,39,20], 'FontSize', 12, 'FontName', 'Yu Gothic UI');
    app.plates(plate_num).filter_fields = uieditfield(tab, 'Visible', Filter_Fields_visibility, 'Position', [87,189,56,22], ...
      'UserData', plate_num, ...
      'ValueChangedFcn', createCallbackFcn(app, @changed_FilterInput_, true) ...
    );

    % Filter Timepoints
    timepoints_label = uilabel(tab, 'Text', 'Timepoints:', 'Visible', Filter_Timepoints_visibility, 'Position', [15,161,67,20], 'FontSize', 12, 'FontName', 'Yu Gothic UI');
    app.plates(plate_num).filter_timepoints = uieditfield(tab, 'Visible', Filter_Timepoints_visibility, 'Position', [87,160,56,22], ...
      'UserData', plate_num, ...
      'ValueChangedFcn', createCallbackFcn(app, @changed_FilterInput_, true) ...
    );

    % Filter Z Slices
    zslices_label = uilabel(tab, 'Text', 'Z Slices:', 'Visible', Filter_zslices_visibility, 'Position', [35,130,47,22], 'FontSize', 12, 'FontName', 'Yu Gothic UI');
    app.plates(plate_num).filter_zslices = uieditfield(tab, 'Visible', Filter_zslices_visibility, 'Position', [87,131,56,22], ...
      'UserData', plate_num, ...
      'ValueChangedFcn', createCallbackFcn(app, @changed_FilterInput_, true) ...
    );
