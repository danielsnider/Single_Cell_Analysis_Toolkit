function fun(app, imgs, segments, subsetTable, plate)
  try
    ...
    for chan_num=[enabled_channel_nums]
      ...
      min_dyn_range_percent = plate.channel_min(chan_num)/100;
      max_dyn_range_percent = plate.channel_max(chan_num)/100;
      ...
      % Set color
      colour = plate.channel_colors(chan_num,:);
    ...
    if app.display.channel_override
      ...
      % Scale image values according to the min max display sliders
      min_dyn_range_percent = plate.channel_min(chan_num);
      max_dyn_range_percent = plate.channel_max(chan_num);
    ...
    for seg_num=1:length(segments)
      ...
      if ~isfield(segments{seg_num},'result') & ~isfield(segments{seg_num},'matrix')
        continue
      end
      if isfield(segments{seg_num},'result')
        seg = segments{seg_num}.result.matrix;
      end
      if isfield(segments{seg_num},'matrix')
        seg = segments{seg_num}.matrix;
      end
      if isempty(seg)
          continue
      end
